...
def _parse_convert_csv_args(
    ...
    parser.arg(
        ...
        type=str,
        required=True,
        help=("Path to training CSV with columns: id, mask, image1 [, image2, ...]."),
    )
    parser.arg(
        "--output",
        type=str,
    ...
    parser.arg(
        ...
        type=str,
        required=False,
        help=("Path to optional test CSV with columns: id, image1 [, image2, ...]."),
    )
    parser.arg(
...
def run_convert_csv(ns: argparse.Namespace) -> None:
    ...
    test_csv = Path(ns.test_csv).expanduser().resolve() if ns.test_csv else None
