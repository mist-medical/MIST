...
class RunTime(object):
    ...
    def decode_dtm(self, serialized_example):
        features_dict = {'dtm': tf.io.VarLenFeature(tf.float32),
                         'dims': tf.io.FixedLenFeature([3], tf.int64),
                         'num_classes': tf.io.FixedLenFeature([1], tf.int64)}
        
        features = tf.io.parse_example(serialized_example, features_dict)
        dtm = tf.sparse.to_dense(features['dtm'])
        tf.reshape(mask, tf.concat([features['dims'], features['num_classes']], axis = -1))
    ...
    def val_inference(self, model_path, df, ds):
        cnt = 0
    ...
    def compute_val_loss(self, model_path, ds):
        K.clear_session()
        gc.collect()
        model = load_model(model_path, compile = False)
        ...
        for element in iterator:
            ...
            for i in range(0, pad_dims[0] - self.inferred_params['patch_size'][0] + 1, strides[0]):
                for j in range(0, pad_dims[1] - self.inferred_params['patch_size'][1] + 1, strides[1]):
                    for k in range(0, pad_dims[2] - self.inferred_params['patch_size'][2] + 1, strides[2]):
                        ...
                        pred_patch = model.predict_on_batch(patch)
                        prediction[i:(i + self.inferred_params['patch_size'][0]),
                                   j:(j + self.inferred_params['patch_size'][1]),
                                   k:(k + self.inferred_params['patch_size'][2]), ...] = pred_patch
            ...
            val_loss.append(dice_loss(truth[..., 0:self.n_classes], prediction))
    ...
    def train(self):
        ...
        for fold in self.params['folds']:
            ...
            for i in range(self.epochs):
                ...
                loss_fn, custom_object = get_loss(self.params,
                                                  class_weights = self.inferred_params['class_weights'], 
                                                  alpha = alpha)
                ...
                if i > 0:
                    if self.multi_gpu:
                        with strategy.scope():
                            ...
                            model = load_model(current_model_name, custom_objects = custom_object)
                            model.compile(optimizer = opt, loss = [loss_fn])
                    else:
                        # Reload model and resume training for later epochs
                        K.clear_session()
                ...
                del train_ds, val_ds, model, loss_fn, custom_object
                K.clear_session()
                gc.collect()
            ...
            self.val_inference(best_model_name, test_df, test_ds)
